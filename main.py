...
def parentChild_exersise():
    ...
    printCourses(save)
    course_index=int(input("enter the course index---"))
    child_ids=[]
    slug_list=[]
    j=0
    while j<len(save["availableCourses"]):
        if course_index==j:
            ...
            print(slug_list)
            print()
            parent_excersis=int(input("enter the parent_excersis index:--"))
            y=0
            while y<len(length):
                if parent_excersis==y:
                    third_api=requests.get("https://merakilearn.org/api/courses/75/exercise/getBySlug?slug="+str(slug_list[y]))
                    break
                y=y+1
